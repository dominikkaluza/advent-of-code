...
final class PuzzleSolutionSecondPart implements PuzzleSolution
{
    public function getResult(): Result
    {
        ...
        foreach ($numbers as $counter => $number) {
            echo "$counter/100\n";
            ...
            foreach ($numbers as $number2) {
                ...
                for($i = 0; $i < 2; $i++) {
                    $first = $i === 0 ? $number : $number2;
                    $second = $i === 0 ? $number2 : $number;

                    $tree = new Node();
                    $tree->addChild($this->buildTree($first));
                    $tree->addChild($this->buildTree($second));
                        ...
                        foreach ($yield as $node) {
                            ...
                            if ($node->getDepth() === 4 && !$node->isLeaf() && count($node->getChildren()) > 0) {
                                ...
                                if (!in_array($leftNeighbour, $visited, true)) {
                                    ...
                                    $leftNeighbour->setValue(
                                        $leftNeighbour->getValue() + $node->getChildren()[0]->getValue()
                                    );
                                }
                                ...
                                if (!in_array($rightNeighbour, $visited, true)) {
                                    ...
                                    $rightNeighbour->setValue(
                                        $rightNeighbour->getValue() + $node->getChildren()[1]->getValue()
                                    );
                                }
        ...
        return IntegerResult::fromArrayMax($magnitudes);
    }
